...
public final class TrainingRecordMapper {
    ...
    public static TrainingRecordDto toDto(TrainingRecord r) {
        var items = r.getItems().stream().map(i ->
                new TrainingRecordItemDto(
                        i.getId().getRecordId(), i.getId().getRb(),
                        i.getExercise().getId(), i.getExercise().getName(),
                        i.getSets(), i.getReps(), i.getWeight()
                )
        ...
        return new TrainingRecordDto(
                r.getId(), r.getTrainingDate(), r.getIntensity(),
                r.getTrainer().getId(), trainerName,
                r.getMember().getId(),  memberName,
                items
        );
    }
}
